...
export default function CarCard({ car }: CarCardProps) {
  const mainImage = getPublicImage(car.image_urls?.[0] ?? null)
    ...
    <div className="group relative bg-brand-navy border border-brand-blue/30 rounded-sm overflow-hidden transition-all duration-700 hover:shadow-[0_20px_50px_rgba(245,124,0,0.1)] hover:-translate-y-2">
      <Link href={`/cars/${car.id}`}>
        <div className="relative aspect-[16/10] overflow-hidden">
          ...
          <img
            ...
            loading="lazy"
            onError={(e) => {
              e.currentTarget.src = "/images/chevroletcar.jpg"
            }}
          />
      ...
      <div className="p-8">
        <div className="mb-6">
          ...
          <p className="text-brand-silver/30 text-[10px] mt-2 uppercase tracking-widest font-medium">
            Showroom Collection • {car.year}
          </p>
        </div>

        <div className="grid grid-cols-2 gap-8 py-6 border-t border-brand-blue/30">
          <div className="space-y-1">
            <span className="text-brand-silver/30 block uppercase tracking-[0.2em] text-[8px] font-bold">
              Mileage
            </span>
            <span className="text-brand-silver text-xs font-bold tracking-tight">
